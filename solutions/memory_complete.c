...
static int get_temperature_from_arduino(char *buffer, size_t buf_len)
{
    ...
    printk(KERN_DEBUG "memory: Opening /dev/ttyACM0\n");

    serial_fp = filp_open("/dev/ttyACM0", O_RDWR | O_NOCTTY, 0);

    if (IS_ERR(serial_fp)) {
        printk(KERN_ERR "memory: Failed to open /dev/ttyACM0, error=%ld\n",
               PTR_ERR(serial_fp));
        return PTR_ERR(serial_fp);
    }

    /*
     * Arduino may reset when the serial port is opened.
     * Wait before sending the command.
     */
    msleep(2000);
